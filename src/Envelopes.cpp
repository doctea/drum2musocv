...
void randomise_envelopes() {
  for (int i = 0 ; i < NUM_ENVELOPES_EXTENDED ; i++) { // _EXTENDED if we want to randomise the extended envelopes too?
    ...
    envelopes[i].lfo_sync_ratio_hold_and_decay = (byte)random(0,127);
    envelopes[i].lfo_sync_ratio_sustain_and_release = (byte)random(0,127);
    envelopes[i].attack_length = (byte)random(0,64);
    envelopes[i].hold_length = (byte)random(0,127);
    envelopes[i].decay_length = (byte)random(0,127);
    envelopes[i].sustain_ratio = 1.0/(float)random(64,127);
    envelopes[i].release_length = (byte)random(0,127);
    envelopes[i].invert = (byte)random(0,10) < 2;
  }
}
