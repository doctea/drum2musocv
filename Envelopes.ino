...
typedef struct envelope_state {
  ...
  long decay_length = 1000;     //  D - decay   - length of stage
  float sustain_ratio = 0.90f;//  S - sustain - level to drop to after decay phase
  long release_length = 4000;   //  R - release - length (time to drop to 0)
...
void update_envelope (byte env_num, byte velocity, bool state) {
  ...
  if (state == true) { //&& envelopes[env_num].stage==OFF) {
    ...
    //envelopes[env_num].last_sent_lvl = -1;
    envelopes[env_num].release_length = 1000;
    envelopes[env_num].decay_length = 500;
    envelopes[env_num].attack_length = 100;
...
void process_envelopes(unsigned long now, unsigned long delta) {
  for (int i = 0 ; i < 1 ; i++) {
    ...
    /*switch (envelopes[i].stage) {
      case SUSTAIN:
        NUMBER_DEBUG(14, envelopes[i].stage, 127); //envelopes[i].stage, lvl);
        break;
    }*/
    ...
    if (s==ATTACK) {
        ...
        if (elapsed >= envelopes[i].attack_length) {
          ...
          //NUMBER_DEBUG(10, envelopes[i].stage, 1);
        }
    } else if (s==DECAY) {
        /*(NOISY_DEBUG(100, 1);
          NOISY_DEBUG(100, 2);
        ...
        NUMBER_DEBUG(14, envelopes[i].stage, envelopes[i].stage_start_level);

        // volume to remain at until note is released
        //float sustain_level = envelopes[i].sustain_ratio * ((float)envelopes[i].stage_start_level);
        //float sustain_level = (float)envelopes[i].stage_start_level;
      ...
      //case RELEASE:
        ...
        if (elapsed >= envelopes[i].release_length) {
          ...
          /*NUMBER_DEBUG(11, envelopes[i].stage, envelopes[i].stage_start_level);
            NUMBER_DEBUG(11, envelopes[i].stage, envelopes[i].stage_start_level);
            NUMBER_DEBUG(11, envelopes[i].stage, envelopes[i].stage_start_level);
            NUMBER_DEBUG(11, envelopes[i].stage, envelopes[i].stage_start_level);
            NUMBER_DEBUG(11, envelopes[i].stage, envelopes[i].stage_start_level);*/
